...
for (const [title, id] of Object.entries(projectImages)) {
  const safeTitle = title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const regex = new RegExp(`(title: "${safeTitle}",[\\s\\S]*?image: ")[^"]+(")`, 'm');
  content = content.replace(regex, `$1https://images.unsplash.com/photo-${id}?w=800&q=80$2`);
}
